...
if [[ -n "${ANTHROPIC_API_KEY:-}" ]]; then
    printf "${_OR}${_B}❯ BashTutor${_R} ${_DG}v%s [claude·%s]${_R}  ${_CY}qq help${_R} to start\n" "$BASHTUTOR_VERSION" "$BASHTUTOR_LEVEL"
else
    printf "${_OR}${_B}❯ BashTutor${_R} ${_DG}v%s [local·%s]${_R}  ${_RE}set ANTHROPIC_API_KEY for AI${_R}\n" "$BASHTUTOR_VERSION" "$BASHTUTOR_LEVEL"
fi
