...
if command -v openclaw &>/dev/null; then
    printf "${_CY}${_B}❯ BashTutor${_R} ${_GR}[openclaw·%s]${_R}  type ${_BL}qq help${_R} to start\n" "$BASHTUTOR_LEVEL"
else
    printf "${_CY}${_B}❯ BashTutor${_R} ${_YE}[local·%s]${_R}  openclaw not found, using patterns\n" "$BASHTUTOR_LEVEL"
fi
...
function _bt_local_lookup() {
    ...
    case "$q" in
        ...
        *port*listen*|*open*port*) _BT_CMD="lsof -i -P | grep LISTEN"; _BT_NOTE_B="Shows open ports"; _BT_NOTE_I="# -P=port numbers" ;;
        ...
        *env*var*|*export*) _BT_CMD="export VAR=value"; _BT_NOTE_B="Sets an environment variable"; _BT_NOTE_I="# add to ~/.zshrc to persist" ;;
        *) return 1 ;;
    esac
    return 0
}
...
function _bt_help() {
    local ai_status
    command -v openclaw &>/dev/null && ai_status="${_GR}openclaw${_R}" || ai_status="${_YE}local fallback${_R}"
