...
if [[ -t 1 ]] && command -v tput &>/dev/null && tput colors &>/dev/null; then
    ...
    _BT_ORANGE=$(tput setaf 214 2>/dev/null || tput setaf 3)
else
...
function bashme() {
    ...
    if [[ "$request" == *" what?" || "$request" == *" what" || "$request" == "what?"* ]]; then
        # Strip the "what?" from the end (or beginning)
...
Watch out: [any gotchas or risks, or skip this if none]"
            ...
            explanation=$(_bashtutor_ask_openclaw "$what_prompt" 20)
        fi
        ...
        if [[ -n "$explanation" ]]; then
            ...
            # Local fallback — explain based on the first word
            local base=$(echo "$cmd_to_explain" | awk '{print $1}')
            local local_exp="${BASHTUTOR_EXPLANATIONS[$base]}"
            if [[ -n "$local_exp" ]]; then
                _bt_explain "$local_exp"
                echo ""
                _bt_tip "For a full breakdown, make sure OpenClaw is running"
            else
                _bt_warn "Not sure what '$base' does — try with OpenClaw running for a full explanation"
            fi
        fi
        echo ""
        return 0
...
if [[ -z "${BASHTUTOR_WELCOME_SHOWN}" ]]; then
    ...
    echo "${_BT_GREEN}${_BT_BOLD}        ╚═╝    ╚═════╝    ╚═╝    ╚═════╝ ╚═╝  ╚═╝${_BT_RESET}"
    echo ""
    echo "        ${_BT_WHITE}${_BT_BOLD}v${BASHTUTOR_VERSION}${_BT_RESET}  ${_BT_ORANGE}🦀 OpenClaw Edition${_BT_RESET}  ${_BT_WHITE}bash for humans${_BT_RESET}"
    echo ""

    if [[ "$BASHTUTOR_AI_AVAILABLE" == "1" ]]; then
        echo "        ${_BT_GREEN}✅ OpenClaw ready${_BT_RESET}   ${_BT_GREEN}👻 Ghost autocomplete on${_BT_RESET}"
    else
